...
export default async function AdminPreviewStudentPage({ params }: { params: Promise<{ classId: string; studentId: string }> }) {
  noStore()
  ...
  const { data: classData } = await admin.from('classes').select('id, name, plan, school_logo_url').eq('id', classId).single()
  ...
  const base = `/admin/classes/${classId}/preview`
